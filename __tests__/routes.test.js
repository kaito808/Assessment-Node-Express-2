...
describe('Middleware: authUser', function () {
  // TESTS BUG #6: Handle invalid token format
  it('should return 401 for invalid token format', async function () {
    const invalidToken = 'invalid_token_format';

    // Send a GET request to a protected route with the invalid token
    const response = await request(app)
      .get('/protected-route')
      .set('Authorization', `Bearer ${invalidToken}`);

    expect(response.statusCode).toBe(401); // Expect a 401 Unauthorized status code
...
describe("POST /auth/register", function () {
  ...
  //  Test case for Bug #1: Disallow user registration with an empty username
  test("should not allow user registration with an empty username", async function() {
    const response = await request(app)
      .post("/auth/register")
      .send({
...
describe("POST /auth/login", function () {
 ...
 //  test case for Bug #2: Disallowing login with an empty username
 test("should disallow login with an empty username", async function() {
  const response = await request(app)
    .post("/auth/login")
    .send({
      username: "", // Empty username
  ...
  expect(response.statusCode).toBe(400); // Expecting a 400 Bad Request error
  expect(response.body).toEqual({
    status: 400,
    message: 'Username cannot be empty' // Check for the error message
  });


 //  test case for Bug #3
 //should not allow user login with an incorrect password without checking for password validity
 test("should not allow login with an incorrect password", async function() {
  const response = await request(app)
    .post("/auth/login")
    .send({
      username: "u1",
...
describe("PATCH /users/[username]", function () {
  ...
  // TESTS BUG #4: Updating user data with an empty username should not be allowed
  test('should not allow updating user data with an empty username', async function() {
    const response = await request(app)
      .patch('/users/u1')
      .send({ _token: tokens.u1, username: '', first_name: 'NewFirstName' });
    ...
    // TESTS BUG #5: Updating the admin field in the user data should not be allowed
  test('should not allow updating the admin field in the user data', async function() {
    const response = await request(app)
      .patch('/users/u1')
    ...
    expect(response.body).toEqual({
      status: 401,
      message: 'Updating the admin field is not allowed',
    });
