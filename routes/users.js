...
router.patch(
  ...
  async function (req, res, next) {
    try {
      ...
      // FIX FOR BUG#4
      // Check if the request body contains a non-empty username
      if (!req.body.username || req.body.username.trim() === "") {
        throw new ExpressError("Username cannot be empty", 400);
      }

      // FIX BUG#5
      // Check if the request body contains the 'admin' field
      if (req.body.admin !== undefined) {
        throw new ExpressError("Updating the admin field is not allowed", 401);
      }
